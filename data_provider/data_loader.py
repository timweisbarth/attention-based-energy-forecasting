...
class SMARD(Dataset):
    ...
    def __read_data__(self):
        ...
        mask_vali = (df_raw['date'].dt.year == 2021) | (df_raw['date'].dt.year == 2022)
        # Substract the indexes for the number of validation elements
        num_vali = df_raw[mask_vali].last_valid_index() - (num_train - 1) 
    ...
    def __getitem__(self, index):
        ...
        # sequence begin/end and response begin/end
        s_begin = index
...
class SMARD_w_WEATHER(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.scale:
            train_data = df_data[border1s[0]:border2s[0]]
            train_weather_data = df_weather[border1s[0]:border2s[0]]
            self.scaler.fit(train_data.values)
            self.scaler_weather.fit(train_weather_data.values)
            data = self.scaler.transform(df_data.values)
            weather = self.scaler_weather.transform(df_weather.values)
        else:
            data = df_data.values
            weather = df_weather.values
